...
{ 
  ...
  //设置DMA1_CH1，用于存储ADC1和ADC2的规则组转换值。
//=================================================================================
...
// 设定TIM1_CH4的工作状态 为比较输出模式,触发AD转换。
  ...
  ADC_InitStructure.ADC_Mode = ADC_Mode_RegSimult; // 工作在同步规则组模式 
  ADC_InitStructure.ADC_ScanConvMode = DISABLE;		 // 多通道模式
  ADC_InitStructure.ADC_ContinuousConvMode = DISABLE; // 连续模式
  ADC_InitStructure.ADC_ExternalTrigConv = ADC_ExternalTrigConv_T4_CC4; //软件触发
  ADC_InitStructure.ADC_DataAlign = ADC_DataAlign_Left;	// 数据左对齐 
  ADC_InitStructure.ADC_NbrOfChannel = 3;  // 3个顺序进行规则转换的 ADC 通道的数目
 ...
 //下面分别设置规则通道组和注入通道组
  ...
  ADC_RegularChannelConfig(ADC1, BRK_SHUNT_CURR_CHANNEL,  2, ADC_SampleTime_13Cycles5); 
  /* ADC1 Regular Channel BUS_SHUNT  */
  ADC_RegularChannelConfig(ADC1, BUS_SHUNT_CURR_CHANNEL,  3, ADC_SampleTime_13Cycles5); 

  /* ADC2 Regular Channel POT1 */ 
  ADC_RegularChannelConfig(ADC2, POT1_VOLT_FDBK_CHANNEL,  1, ADC_SampleTime_13Cycles5); 
  /* ADC2 Regular Channel AIN0 */ 
  ADC_RegularChannelConfig(ADC2, AIN0_VOLT_FDBK_CHANNEL,  2, ADC_SampleTime_13Cycles5); 
   /* ADC1 Regular Channel AIN1 */ 
  ADC_RegularChannelConfig(ADC2, AIN1_VOLT_FDBK_CHANNEL,  3, ADC_SampleTime_13Cycles5); 
...
//  NVIC_StructInit(&NVIC_InitStructure);
  ...
  NVIC_InitStructure.NVIC_IRQChannelCmd = DISABLE;	//没有打开紧急停车功能
  NVIC_Init(&NVIC_InitStructure);
...
/* Enable the DMA_CHANEL1 Interrupt */
  ...
  /* ADC1、ADC2 regular conversions trigger is software*/ 
