...
/* Private define ------------------------------------------------------------*/
#define HALL_MAX_SPEED_FDBK (u16)(HALL_MAX_SPEED_FDBK_RPM/6 * POLE_PAIR_NUM) //2500	 µ¥Î»£ºHZ
#define HALL_MIN_SPEED_FDBK (u16)(HALL_MIN_SPEED_FDBK_RPM/6* POLE_PAIR_NUM)	 //5	 µ¥Î»£ºHZ
...
vu8 bGP1_OVF_Counter;   //ÓÉÓÚ·ÖÆµÌ«µÍµ¼ÖÂ¼ÆÊýÒç³öµÄ´ÎÊý
vu16 hCaptCounter;      //²¶»ñÖÐ¶Ï·¢ÉúµÄ´ÎÊý
...
static u8 pole_paris = 0;  //¸Ã±äÁ¿ÓÃÓÚ²âÊÔ¼«¶ÔÊý¡£
#if (defined HALL_SENSORS || defined VIEW_HALL_FEEDBACK)
static s8 bSpeed = POSITIVE;
#endif
...
//    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU;
    ...
    TIM_HALLTimeBaseInitStructure.TIM_ClockDivision = TIM_CKD_DIV1;	//ÉèÖÃÊý×ÖÂË²¨Ê±¼ä·ÖÆµÊý
    ...
    TIM_HALLICInitStructure.TIM_ICPolarity = TIM_ICPolarity_Falling; //Ã¿×ª120µç½Ç¶È£¬²úÉúÒ»´ÎÏÂ½µÑØ¡£
    TIM_HALLICInitStructure.TIM_ICFilter = ICx_FILTER;				 //ËùÒÔÃ¿²úÉúÒ»´Î²¶»ñÖÐ¶Ï£¬Ïàµ±ÓÚÂí´ï×ªÁË120µç½Ç¶È¡£
    																 //µ«ÊÇ»»ÏòÊÇÃ¿60µç½Ç¶È¾ÍÖ´ÐÐÒ»´Î£¬ÒòÎªÈý¸öÊäÈëÐÅºÅ
    TIM_ICInit(HALL_TIMER,&TIM_HALLICInitStructure);				 //ÊÇXORºóÈ»ºóÊäÈëCHANEL1µÄ¡£
...
//    TIM_InternalClockConfig(HALL_TIMER);
	...
	//¸üÐÂÖÐ¶ÏÔ´ÊÇ¼ÆÊýÆ÷ÉÏÒç»òÕßÏÂÒç£¬»¹ÓÐÈí¼þUGÎ»ÒÔ¼°Íâ²¿´¥·¢ÒýÆðµÄUPDATE.
...
void HALL_InitHallMeasure( void )
{
   ...
   hCaptCounter = 0;	 //²¶»ñÖÐ¶Ï·¢ÉúµÄ´ÎÊý
   bGP1_OVF_Counter = 0; //ÓÉÓÚ·ÖÆµÌ«µÍµ¼ÖÂ¼ÆÊýÒç³öµÄ´ÎÊý
...
//      SensorPeriod[bSpeedFIFO_Index].bDirection = NEGATIVE;
   ...
   HALL_TIMER->PSC = HALL_MAX_RATIO;   //³õÊ¼»¯·ÖÆµÊý¡£500
...
/*******************************************************************************
* ROUTINE Name : HALL_GetSpeed	   µ¥Î»£ºHZ 
*
...
{ 
  ...
  if( hRotorFreq_dpp == HALL_MAX_PSEUDO_SPEED)	//Ìõ¼þÊÇ£º
  												//£¨PeriodMeasAux.wPeriod < (u32)SPEED_OVERFLOW )
  {
    return (HALL_MAX_SPEED);   //×î¸ß×ªËÙ£¬
  }
  else
  {
   ...
   else
   {
     ...
     //No errors have been detected during rotor speed information extrapolation          
     {
        ...
        {                           /* Based on current prescaler value only */
           hRotorFreq_dpp = 0;	   //Âí´ïÆô¶¯»òÕß×ªËÙ·Ç³£µÍ
        }
        else
        {							   //×ªËÙÌ«µÍ
           ...
           {  /*Avoid u32 DIV Overflow*/
              if ( PeriodMeasAux.wPeriod > (u32)SPEED_OVERFLOW )  
              {
                ...
                {
                  hRotorFreq_dpp=0;	 //µÚÒ»´ÎµÄ¼ÆÊýÆ÷²¶×½¶ªÆú
                }										   //×î¸ßËÙ			  //×îµÍËÙ
                else              //PSEUDO_FREQ_CONV  ½éÓÚ SPEED_OVERFLOWºÍ MAX_PERIODÖ®¼ä¡£  
                {
              ...
              {
                hRotorFreq_dpp = HALL_MAX_PSEUDO_SPEED;	  //-32768 ËÙ¶ÈÌ«¸ßÁË
              }
           }
        }
     }          
...
PeriodMeas_s GetLastHallPeriod(void)
{
   ...
   bLastSpeedFIFO_Index = bSpeedFIFO_Index;	//·ÀÖ¹ÖÐ¶Ï·¢Éú¸Ä±äË÷ÒýºÅ
...
// Check for the source of TIMx int - Capture or Update Event - 
  if ( TIM_GetFlagStatus(HALL_TIMER, TIM_FLAG_Update) == RESET )
  {
	...
	// ²¶»ñÖÐ¶Ï£¬Ö÷ÒªÊÇÐÞ¸Äµç½Ç¶ÈºÍÂí´ï×ªÏò
    bPrevHallState = bHallState;
    bHallState = ReadHallState();
#if (HALL_SENSORS_PLACEMENT == DEGREES_120)    
    switch(bHallState)
    {
      case STATE_5:	//¶ÔÓ¦ÓÚÁã¶Èµç½Ç¶È
	 ...
	 //a speed reversal occured 
        ...
        {
          hElectrical_Angle = S16_PHASE_SHIFT;     	//0¶Èµç½Ç¶È
        }
        break;
    ...
    case STATE_3:
        ...
        else if(bSpeed!= ERROR)		//120µç½Ç¶È
        {
          hElectrical_Angle =(s16)(S16_PHASE_SHIFT + S16_120_PHASE_SHIFT);
        }
        break;  
    ...
    switch(bHallState)
    {
      ...
      case STATE_6:
        ...
        else if(bSpeed!= ERROR)			//180µç½Ç¶È
        {
          hElectrical_Angle =(s16)(S16_PHASE_SHIFT + S16_120_PHASE_SHIFT+
                                                            S16_60_PHASE_SHIFT);
        }
...
// ÒÔÉÏÖ÷ÒªÊÇÐÞ¸Äµç½Ç¶ÈºÍÂí´ï×ªÏò

   // A capture event occured, it clears the flag £¬²¶»ñÖÐ¶Ï	
	TIM_ClearFlag(HALL_TIMER, TIM_FLAG_CC1);
   
   // used for discarding first capture
   if (hCaptCounter < U16_MAX)
   {
      hCaptCounter++;	    //²¶»ñ´ÎÊý£¬Âí´ïÃ¿×ª120µç½Ç¶È£¬ÖÐ¶ÏÒ»´Î¡£
	  						//ËùÒÔhCaptCounterÃ¿Ôö¼ÓÈý£¬¾Í×ª360¶Èµç½Ç¶È¡£
   }
   ...
   {
      bSpeedFIFO_Index = 0;	   //´æµ½µÚÒ»¸öÎ»ÖÃ
   }
   ...
   if (bGP1_OVF_Counter != 0)	// There was counter overflow before capture
   {
      ...
      wCaptBuf = (u32)TIM_GetCapture1(HALL_TIMER);   //µÃµ½×ª120µç½Ç¶ÈµÄ¼ÆÊýÖµ     
      ...
      SensorPeriod[bSpeedFIFO_Index].bDirection = bSpeed; //ÓÐ4ÖÖ×´Ì¬
      if (RatioInc)
      {
   ...
   }  //ÒÔÉÏÊÇÓÉÓÚ·ÖÆµÌ«µÍ£¬µ¼ÖÂ¼ÆÊýÒç³öµÄ´¦Àí
   else		// ÎÞÒç³ö
   {
      u16 hHighSpeedCapture, hClockPrescaler;   

      hHighSpeedCapture = (u32)TIM_GetCapture1(HALL_TIMER);	 //µÃµ½×ª120µç½Ç¶ÈµÄ¼ÆÊýÖµ
   ...
   if (InitRollingAverage)	 //Ö»Òª·¢Éú²âµÃÒ»´Î²¶»ñµÄÊý¾Ý£¬¾Í¿ÉÒÔ¼ÆËãÆ½¾ùÖµ
   {
      ...
      DoRollingAverage = TRUE;	//¿ÉÒÔ¼ÆËãËÙ¶ÈÆ½¾ùÖµµÄ±êÖ¾
    }
   
  //Update Rotor Frequency Computation
   hRotorFreq_dpp = HALL_GetRotorFreq(); //¼ÆËãSPEED_DPP¸ñÊ½µÄÊý¾Ý
  										 //Ã¿120µç½Ç¶È¼ÆËãÒ»´Î¡£
  }										 //
  else 	//Òç³öÖÐ¶Ï£¬Ô¤·ÖÆµÖµÌ«Ð¡£¬½á¹ûµ¼ÖÂ¼ÆÊýÆ÷>U16_MAX
  {
    ...
    if (bGP1_OVF_Counter >= HALL_MAX_OVERFLOWS)	  //2´ÎÒç³ö¾ÍÊ§°Ü
//    if (bGP1_OVF_Counter >= 6 )	  //2´ÎÒç³ö¾ÍÊ§°Ü
    {
       HallTimeOut = TRUE;
       hRotorFreq_dpp = 0;
    }    
