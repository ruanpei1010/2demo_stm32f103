...
void ENC_Init(void)
{
  ...
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING;	   //设定内部上拉
 ...
 // Clear all pending interrupts
  ...
  //Reset counter
  ENCODER_TIMER->CNT = COUNTER_RESET; // 是否有问题，应该为：ENCODER_TIMER->CNT = COUNTER_RESET
...
{
  //Reset counter
  TIM2->CNT = COUNTER_RESET;   // 是否有问题，应该为：ENCODER_TIMER->CNT = COUNTER_RESET
}
